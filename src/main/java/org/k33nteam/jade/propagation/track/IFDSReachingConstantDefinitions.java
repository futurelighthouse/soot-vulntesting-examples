...
public class IFDSReachingConstantDefinitions extends DefaultJimpleIFDSTabulationProblem<TrackFact,InterproceduralCFG<Unit, SootMethod>> {
	...
	final static boolean CONSTANT_DEBUG = true;
		...
		return new FlowFunctions<Unit, TrackFact, SootMethod>() {
						...
						public Set<TrackFact> computeTargets(TrackFact source) {
							...
							if (source != zeroValue()) {
								...
								if (source.getO1().equivTo(assignment.getLeftOp()) || maybeSameLocation(source.getO1(), assignment.getLeftOp())) {//to save time
									if(DEBUG) System.out.println("in def statement transfering from " + source + " to " + assignment.getRightOp());
				...
				if (curr instanceof ReturnStmt) {
					...
					return new FlowFunction<TrackFact>() {
						...
						public Set<TrackFact> computeTargets(TrackFact source) {
							...
							if (source != zeroValue()) {
								//Maybe this block is not needed
								if ( maybeSameLocation(returnStmt.getOp(), source.getO1())) {
									...
									if(DEBUG) System.out.println("in return statementtransfering from " + source + " to " + returnStmt.getOp());
			...
			public FlowFunction<TrackFact> getCallFlowFunction(Unit callStmt,
				...
				return new FlowFunction<TrackFact>() {
					...
					@Override
					public Set<TrackFact> computeTargets(TrackFact source) {
						if(DEBUG) System.out.println("getCallFlowFunction: " + stmt + " source: " + source +" in method: " + interproceduralCFG().getMethodOf(stmt) + "dst method " + destinationMethod);
						...
						if (source ==  zeroValue()) {
							...
							if (stmt instanceof DefinitionStmt) //call statement has assignment, need to check
							{
									...
									for (Unit u : destinationMethod.getActiveBody().getUnits()) {
										if (u instanceof ReturnStmt) {
											...
											if (rStmt.getOp() instanceof Local
												...
												values.add(new TrackFact(rStmt.getOp(), source.getO2()));//we are now tracking the returned value
												//and propagate through function
												//notice: backward!
											}
											else if (rStmt.getOp() instanceof Constant || rStmt.getOp() instanceof StaticFieldRef) {
												//directly constant, HOOK here
												//no need to propagate backward anymore
							...
							if (values.size() == 0) {
								if (DEBUG) {
									System.out.println("killing value: " + source + " in callflowfunction because no corresponding source to original source");
								}
								return Collections.emptySet();
							}
							else {
			...
			public FlowFunction<TrackFact> getCallToReturnFlowFunction(final Unit callSite, final Unit returnSite) {
				...
				if(DEBUG) System.out.println("calltoreturnflow :"+stmt);
				/*
				...
				return new FlowFunction<TrackFact>() {
					...
					private Set<TrackFact> processPotentialGeneration(Stmt stmt, TrackFact source)
					{
						if (stmt.containsInvokeExpr()) {
							if (DEBUG) {
								System.out.println("callsite: " + callSite + " of method: " + interproceduralCFG().getMethodOf(callSite) + " factsource: " + source);
							}
					...
					private TrackFact processCallToReturnFact(TrackFact source)
					{
						...
						if (callSite instanceof DefinitionStmt) {
							...
							if(source.getO1().equivTo(assignStmt.getLeftOp()) || maybeSameLocation(source.getO1(), assignStmt.getLeftOp())) {
								...
								System.out.println("normal return in calltoreturnflow, source " + source);
								return source;
							}
						}
						System.out.println("normal return without def stmt in calltoreturnflow, source " + source);
						return source;
					}
				};
			}
